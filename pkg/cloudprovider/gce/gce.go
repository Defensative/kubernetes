...
// GCECloud is an implementation of Interface, TCPLoadBalancer, UDPLoadBalancer, and Instances for Google Compute Engine.
...
func (gce *GCECloud) UDPLoadBalancer() (cloudprovider.UDPLoadBalancer, bool) {
	return gce, true
}
...
// GetUDPLoadBalancer is an implementation of UDPLoadBalancer.GetUDPLoadBalancer
func (gce *GCECloud) GetUDPLoadBalancer(name, region string) (*api.LoadBalancerStatus, bool, error) {
...
func CreateLoadBalancer(gce *GCECloud, name, region string, externalIP net.IP, ports []*api.ServicePort, hosts []string, affinityType api.ServiceAffinity, protocol string) (*api.LoadBalancerStatus, error) {
    if protocol != "TCP" && protocol != "UDP" {
        return nil, fmt.Errorf("%s is an invalid protocol", protocol)
    }
	...
	targetTags := gce.computeHostTags(hosts)
...
// CreateUDPLoadBalancer is an implementation of UDPLoadBalancer.CreateUDPLoadBalancer.
// TODO(a-robinson): Don't just ignore specified IP addresses. Check if they're
// owned by the project and available to be used, and use them if they are.
func (gce *GCECloud) CreateUDPLoadBalancer(name, region string, externalIP net.IP, ports []*api.ServicePort, hosts []string, affinityType api.ServiceAffinity) (*api.LoadBalancerStatus, error) {
    return CreateLoadBalancer(gce, name, region, externalIP, ports, hosts, affinityType, "UDP");
}
...
func (gce *GCECloud) CreateTCPLoadBalancer(name, region string, externalIP net.IP, ports []*api.ServicePort, hosts []string, affinityType api.ServiceAffinity) (*api.LoadBalancerStatus, error) {
    return CreateLoadBalancer(gce, name, region, externalIP, ports, hosts, affinityType, "TCP");
}

// This will add all tags that are a prefix of the host, which should handle the GKE case.
// This will not add any other tags, so in the case of just regular GCE no tags will be added
func (gce *GCECloud) computeHostTags(hosts []string) []string {
    var targetTags []string
	for _, host := range hosts {
        instance, err := gce.getInstanceByName(host)
        if instance == nil {
            glog.Errorf("Failed to lookup host: %s", host);
        } else if err != nil {
            glog.Errorf("Error looking up host: %s (%s)", host, err.Error())
        } else if instance.Tags == nil || len(instance.Tags.Items) == 0 {
            glog.Infof("Host: %s has 0 tags, not providing tags for fw", host)
        } else {
            glog.Infof("Host %s has tags: %s", host, instance.Tags)
            for _, tag := range instance.Tags.Items {
                if strings.HasPrefix(host, tag) {
                    glog.Infof("Adding tag: %s for Host %s, as it is the base instance tag", tag, host)
...
func UpdateLoadBalancer(gce *GCECloud, name, region string, hosts []string) error {
...
func (gce *GCECloud) UpdateTCPLoadBalancer(name, region string, hosts []string) error {
    return UpdateLoadBalancer(gce, name, region, hosts);
}

// UpdateUDPLoadBalancer is an implementation of UDPLoadBalancer.UpdateUDPLoadBalancer.
func (gce *GCECloud) UpdateUDPLoadBalancer(name, region string, hosts []string) error {
    return UpdateLoadBalancer(gce, name, region, hosts);
}

func EnsureLoadBalancerDeleted(gce *GCECloud, name, region string) error {
